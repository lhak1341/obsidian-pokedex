import encounterLocationsData from "./encounterLocations.json";
...
// Wild-encounter location areas, keyed by RawPokemon.name (not dex number —
// see PokedexTableRow.encounterLocations for why: a regional-form row shares
// its base species' dexNumber but needs its own key, even though this v1
// table only ever populates the base species' key). Generated by
// scripts/generate-encounters.ts; regenerate after a new generation ships
// (new location areas/versions) the same way EVOLUTION_STAGES gets
// regenerated after a new set of evolution chains.
// TS infers a plain `string` for the JSON's `method` field (it sees several
// different string values across the file, not one repeated literal), which
// doesn't satisfy the narrower union below on its own — hence the cast.
// Regenerating via scripts/generate-encounters.ts is the only thing that
// writes this file, and it only ever writes one of these four values.
export const ENCOUNTER_LOCATIONS: Readonly<
	Record<string, {
		name: string;
		generationId: number;
		region: string | null;
...
> =
	encounterLocationsData as Record<
		string,
		{ name: string; generationId: number; region: string | null; method: "trade" | "gift" | "egg" | null; tradeFor: string | null }[]
	>;
