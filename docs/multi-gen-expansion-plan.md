...
3. **New evolution conditions**: check whether this generation introduces evolution triggers not yet modeled (see each phase's bullet list above — e.g. Gen 5 adds `trade_species`, Gen 8 adds `needs_overworld_rain`/`turn_upside_down`). Extend `RawEvolutionChainLink`/`EvolutionNode` in `types.ts`, `normalizeEvolutionChain` in `normalize.ts`, and `detailLabel` in `EvolutionChain.svelte` — same pattern each time (raw field → normalized field → label branch), fix test fixtures' literal `EvolutionNode` objects to match the added fields.
